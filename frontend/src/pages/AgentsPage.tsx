...
import type { Agent, AgentPurpose, AgentTone, DeploymentChannel } from '@/types';

const templates: Array<{ id: string; icon: typeof Sparkles; name: string; description: string; purpose: AgentPurpose }> = [
  { id: 'blank', icon: Sparkles, name: 'Start from scratch', description: 'Build a custom agent your way.', purpose: 'support' },
  { id: 'support', icon: MessageSquare, name: 'Customer support', description: 'Resolve product and account questions.', purpose: 'support' },
  { id: 'lead', icon: Bot, name: 'Lead qualification', description: 'Capture and qualify inbound leads.', purpose: 'lead_generation' },
  { id: 'education', icon: BookOpen, name: 'Education', description: 'Teach enrolled learners from trusted course knowledge.', purpose: 'education' },
];

const purposeLabels: Record<AgentPurpose, string> = {
...
function AgentCard({ agent, onDuplicate, onDelete }: { agent: Agent; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <Card className="agent-card">
    <div className="agent-card__top"><span className="agent-avatar agent-avatar--lg" style={{ background: agent.appearance.primaryColor }}>{agent.avatar}</span><Badge tone={agent.status === 'active' ? 'success' : agent.status === 'training' ? 'purple' : 'warning'}><i className="status-dot" /> {agent.status}</Badge><Badge tone="brand">{purposeLabels[agent.purpose]}</Badge><div className="more-menu" ref={ref}><button className="icon-button" onClick={() => setOpen((value) => !value)} aria-label={`Actions for ${agent.name}`}><MoreHorizontal /></button>{open ? <div className="menu-popover"><button onClick={() => { setOpen(false); onDuplicate(); }}><Copy />Duplicate</button><button className="danger" onClick={onDelete}><Trash2 />Delete</button></div> : null}</div></div>
...
function CreateAgentModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (agent: Agent) => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="Create an AI agent" description="Follow four simple steps. You can adjust every setting later." size="lg" footer={<><Button variant="secondary" onClick={step === 0 ? onClose : () => setStep((current) => current - 1)} icon={step === 0 ? undefined : ChevronLeft}>{step === 0 ? 'Cancel' : 'Back'}</Button><Button disabled={!canContinue || saving} icon={step === steps.length - 1 ? Check : ChevronRight} onClick={advance}>{saving ? 'Creating…' : step === steps.length - 1 ? 'Create agent' : 'Continue'}</Button></>}>
    <form className="agent-flow" onSubmit={(event) => { event.preventDefault(); advance(); }}>
      ...
      {step === 1 ? <section className="agent-flow__panel"><div className="agent-flow__intro"><Bot /><div><h3>Choose its main job</h3><p>The selected purpose is stored on the Agent and will control purpose-specific runtime behavior as those workflows are enabled.</p></div></div><fieldset className="template-field"><legend>Agent purpose and starting point</legend><div className="template-grid">{templates.map(({ id, icon: Icon, name: templateName, description: text, purpose }) => <label key={id} className={template === id ? 'is-selected' : ''}><input type="radio" name="template" value={id} checked={template === id} onChange={() => setTemplate(id)} /><Icon /><strong>{templateName}</strong><small>{text}</small><small>Purpose: {purposeLabels[purpose]}</small></label>)}</div></fieldset></section> : null}
